...
Citizen.CreateThread(function()
	while true do
		...
		local inRange = false
		local distance = GetDistanceBetweenCoords(pos, 3615.58, 3732.16, 28.69, true)
		if distance < 10 then
			inRange = true
			if distance < 1.5 then
				DrawText3D(3615.58, 3732.16, 28.69, "~g~E~w~ - Craft")
...
function ItemsToItemInfo()
	itemInfos = {
		...
		[12] = {costs = QBCore.Shared.Items["metalscrap"]["label"] .. ": 80x, " ..QBCore.Shared.Items["plastic"]["label"] .. ": 82x. "},	
	}
