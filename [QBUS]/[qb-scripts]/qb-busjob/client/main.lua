...
Citizen.CreateThread(function() 
	...
	table.insert(JobsClientCore.MarkerList, 
		{name="Bus_DelVeh",
			...
			r=204,g=50,b=50,
			MSizex=1.5,MSizey=1.5,MSizez=10,
			MarkerHelpTekst="Remove your bus",
...
function RunRoute(Route)
	...
	JobsClientCore.RunRoute(Route, {r=189,g=214,b=69,Blip=24,cat=2}, 20.5, 50.5,
		...
		function(location) --At location
			...
			AddTextComponentString("~INPUT_DETONATE~ Door open")
				...
				if(LastRoute[#LastRoute].id == location.id) then
					...
					for ListID=#PedsInsideBus,1,-1 do
						...
						if (GetOutOfBusChange < math.random(1,100)) then
							...
							print("Sit down: "..ListID)
						end
					end
				end
			end
			if(#PedsToGoOut > 0) then
				TriggerServerEvent("InteractSound_SV:PlayWithinDistance", AfspeelAfstand, "stopknopje", 0.25)
				stop('visible')
			end
		end, 
			...
			if (#GetPedsInBus() >= 1)then
				Citizen.CreateThread(function() 
					if(LastRoute[#LastRoute].id == location.id) then
						...
						TriggerServerEvent("InteractSound_SV:PlayWithinDistance", AfspeelAfstand, "eindelijn", 0.5)
						Citizen.Wait(10000)
					else
					...
					if(LastSaid ~= true) then
						LastSaid = true
						TriggerServerEvent("InteractSound_SV:PlayWithinDistance", AfspeelAfstand, "vergeetniet", 0.5)
					else
						LastSaid = false
					end
				end)
