...
RegisterServerEvent('QBCore-diving:server:SetBerthVehicle')
AddEventHandler('QBCore-diving:server:SetBerthVehicle', function(BerthId, vehicleModel)
    TriggerClientEvent('QBCore-diving:client:SetBerthVehicle', -1, BerthId, vehicleModel)
    
    QBCoreBoatshop.Locations["berths"][BerthId]["boatModel"] = boatModel
end)

RegisterServerEvent('QBCore-diving:server:SetDockInUse')
AddEventHandler('QBCore-diving:server:SetDockInUse', function(BerthId, InUse)
    QBCoreBoatshop.Locations["berths"][BerthId]["inUse"] = InUse
    TriggerClientEvent('QBCore-diving:client:SetDockInUse', -1, BerthId, InUse)
end)

QBCore.Functions.CreateCallback('QBCore-diving:server:GetBusyDocks', function(source, cb)
    cb(QBCoreBoatshop.Locations["berths"])
end)

RegisterServerEvent('QBCore-diving:server:BuyBoat')
AddEventHandler('QBCore-diving:server:BuyBoat', function(boatModel, BerthId)
    ...
    if PlayerMoney.cash >= BoatPrice then
        ...
        TriggerClientEvent('QBCore-diving:client:BuyBoat', src, boatModel, plate)
...
QBCore.Functions.CreateUseableItem("jerry_can", function(source, item)
    ...
    TriggerClientEvent("QBCore-diving:client:UseJerrycan", source)
end)

QBCore.Functions.CreateUseableItem("diving_gear", function(source, item)
    local Player = QBCore.Functions.GetPlayer(source)

    TriggerClientEvent("QBCore-diving:client:UseGear", source, true)
end)

RegisterServerEvent('QBCore-diving:server:RemoveItem')
AddEventHandler('QBCore-diving:server:RemoveItem', function(item, amount)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
QBCore.Functions.CreateCallback('QBCore-diving:server:GetMyBoats', function(source, cb, dock)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
QBCore.Functions.CreateCallback('QBCore-diving:server:GetDepotBoats', function(source, cb, dock)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
RegisterServerEvent('QBCore-diving:server:SetBoatState')
AddEventHandler('QBCore-diving:server:SetBoatState', function(plate, state, boathouse)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
RegisterServerEvent('QBCore-diving:server:CallCops')
AddEventHandler('QBCore-diving:server:CallCops', function(Coords)
    local src = source
    for k, v in pairs(QBCore.Functions.GetPlayers()) do
        ...
        if Player ~= nil then
            if (Player.PlayerData.job.name == "police" and Player.PlayerData.job.onduty) then
                ...
                TriggerClientEvent('QBCore-diving:client:CallCops', Player.PlayerData.source, Coords, msg)
                local alertData = {
                    title = "illegal diving",
                    coords = {x = Coords.x, y = Coords.y, z = Coords.z},
                    description = msg,
                }
                TriggerClientEvent("QBCore-phone:client:addPoliceAlert", -1, alertData)
            end
        end
	end
end)
...
QBCore.Commands.Add("wetsuit", "take or put on wetsuit", {}, false, function(source, args)
    local Player = QBCore.Functions.GetPlayer(source)
    TriggerClientEvent("QBCore-diving:client:UseGear", source, false)
end)

RegisterServerEvent('QBCore-diving:server:SellCoral')
AddEventHandler('QBCore-diving:server:SellCoral', function()
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
