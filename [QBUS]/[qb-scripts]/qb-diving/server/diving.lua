...
QBCore.Functions.CreateCallback('QBCore-diving:server:GetDivingConfig', function(source, cb)
    cb(QBCoreDiving.Locations, CurrentDivingArea)
end)

RegisterServerEvent('QBCore-diving:server:TakeCoral')
AddEventHandler('QBCore-diving:server:TakeCoral', function(Area, Coral, Bool)
    local src = source
    ...
    if (QBCoreDiving.Locations[Area].TotalCoral - 1) == 0 then
        ...
        TriggerClientEvent('QBCore-diving:client:NewLocations', -1)
    else
    ...
    TriggerClientEvent('QBCore-diving:server:UpdateCoral', -1, Area, Coral, Bool)
end)

RegisterServerEvent('QBCore-diving:server:RemoveGear')
AddEventHandler('QBCore-diving:server:RemoveGear', function()
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
RegisterServerEvent('QBCore-diving:server:GiveBackGear')
AddEventHandler('QBCore-diving:server:GiveBackGear', function()
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
