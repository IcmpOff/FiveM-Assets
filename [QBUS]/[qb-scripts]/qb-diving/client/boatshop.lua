            ...
            if BuyDistance < 2 then                
                ...
                if not Buying then
                    ...
                    if IsControlJustPressed(0, Keys["7"]) or IsDisabledControlJustReleased(0, Keys["7"]) then
                        TriggerServerEvent('QBCore-diving:server:BuyBoat', QBCoreBoatshop.Locations["berths"][ClosestBerth]["boatModel"], ClosestBerth)
                        Buying = false
...
RegisterNetEvent('QBCore-diving:client:BuyBoat')
AddEventHandler('QBCore-diving:client:BuyBoat', function(boatModel, plate)
    DoScreenFadeOut(250)
    Citizen.Wait(250)
